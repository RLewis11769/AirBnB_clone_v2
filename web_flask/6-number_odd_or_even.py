...
def checkIntTemplate(char):
    """ Displays Number: char only if char is an int """
    # template requires {{ value }} for variables (must pass in)
    return render_template('5-number.html', value=char)


@app.route('/number_odd_or_even/<int:char>', strict_slashes=False)
def EvenOdd(char):
    """ Displays char is even/odd if char is an int """
    # logic for {{ value }} odd or even is in template (Jinja)
    return render_template('6-number_odd_or_even.html', value=char)
